use std::thread;
use std::env;
use crate::pfee::{HeliusClient, QuickNodeClient, AlchemyClient};
use {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let slack_key = "SLACK_WEBHOOK";
	match std::env::var(slack_key) {
		Ok(val) => {
			if val.starts_with("https://hooks.slack.com/") {
		...
		Err(e) => warn!("Couldn't interpret {slack_key}: {e}. Slack messaging service unavailable."),
	}

	let mut exists_discord_webhook = false;
	let mut discord_webhook = String::new();

	let discord_key = "DISCORD_WEBHOOK";
	match std::env::var(discord_key) {
		Ok(val) => {
			if val.starts_with("https://discord.com/api/webhooks/") {
				exists_discord_webhook = true;
				discord_webhook = val;
			} else {
				warn!("Invalid Discord webhook URL format.");
			}
		},
		Err(e) => warn!("Couldn't interpret {discord_key}: {e}. Discord messaging service unavailable."),
	}

	if exists_slack_webhook {
		println!("\n\n\nValid Slack webhook found: {}\n", slack_webhook);
	} else {
		println!("No valid Slack webhook found.");
	}

	if exists_discord_webhook {
		println!("Valid Discord webhook found: {}\n\n", discord_webhook);
	} else {
		println!("No valid Discord webhook found.");
	}
                    ...
                    if solution.is_some() {
                        ...
                        for i in 0..SUBMIT_LIMIT {
                            if let Some(best_solution) = best_solution {
								...
								let helius_client = HeliusClient {
									api_url: "https://mainnet.helius-rpc.com/v1/estimatePriorityFees".to_string(),
								};


								let quiknode_client = QuickNodeClient {
									api_url: env::var("QUICKNODE_URL").expect("QUICKNODE_URL must be set in .env").to_string(),
								};

								let alchemy_client = AlchemyClient {
									api_url: "https://solana-mainnet.alchemyapi.io".to_string(),
								};

								match pfee::dynamic_fee(&rpc_client, (*app_dynamic_fee_url).clone(), &helius_client, &quiknode_client, &alchemy_client,
                                        ...
                                        .await
                                    {
                                        Ok(sig) => {
											...
											if !*app_no_sound_notification {
												if difficulty > 25 {
													// Spiele den High-Difficulty-Sound in einem separaten Thread ab
													thread::spawn(|| {
														utils::play_high_difficulty_sound();  // Spiele den speziellen Sound für hohe Difficulty ab
													});
												} else {
													// Spiele den Standard-Sound in einem separaten Thread ab
													thread::spawn(|| {
														utils::play_sound();  // Der Standard-Sound
													});
												}
											}
...
async fn ws_handler(
    ...
    let now = SystemTime::now()
        ...
        .map(|dur| dur.as_secs())
        .unwrap_or_else(|e| {
            tracing::error!("Systemzeit ist inkonsistent: {:?}", e);
            0 // Fallback-Wert, sollte sicher gehandhabt werden
        });

    if now.saturating_sub(msg_timestamp) >= 30 {
        return Err((StatusCode::UNAUTHORIZED, "Timestamp too old."));
    }

    let user_pubkey = match Pubkey::from_str(pubkey) {
        Ok(pubkey) => pubkey,
        Err(_) => return Err((StatusCode::UNAUTHORIZED, "Invalid pubkey")),
    };

    {
        let already_connected = app_state.read().await.sockets.iter().any(|(_, client_connection)| {
            client_connection.pubkey == user_pubkey
        });
    ...
    let signature = match Signature::from_str(signed_msg) {
        Ok(sig) => sig,
...
async fn handle_socket(
    ...
    if let Err(e) = socket.send(axum::extract::ws::Message::Ping(vec![1, 2, 3])).await {
        error!("Konnte {who} mit Pubkey {who_pubkey} nicht pingen: {e}");
        return;
    }
    tracing::debug!("Ping an {who} mit Pubkey {who_pubkey} gesendet.");
...

    {
        ...
        if app_state.sockets.contains_key(&who) {
            info!("Socket-Adresse {who} hat bereits eine aktive Verbindung.");
            return;
        } else {
    ...
    } // Schreibzugriff wird hier automatisch freigegeben
    ...
    let process_handle = tokio::spawn(async move {
        loop {
            match receiver.next().await {
                ...
                Some(Err(e)) => {
                    error!("Fehler beim Empfangen einer Nachricht von {who}: {e}");
                    break;
                }
                None => {
                    info!("Verbindung von {who} wurde geschlossen.");
                    break;
                }
            }
        }
    ...
    if let Err(e) = process_handle.await {
        error!("Der Prozess-Task für {who} ist mit einem Fehler abgebrochen: {e}");
    }
    ...
    info!("Client mit Pubkey {who_pubkey} hat die Verbindung getrennt.");
}
